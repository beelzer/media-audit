...
from ..models import EpisodeItem, SeasonItem, SeriesItem, VideoInfo
...
class TVParser(BaseParser):
    ...
    def create_episode(self, video_path: Path, ep_info: dict) -> EpisodeItem | None:
    ...
    def find_season_directories(self, directory: Path) -> list[Path]:
        ...
        for item in directory.iterdir():
            if item.is_dir():
                if self.extract_season_number(item.name) is not None:
                    season_dirs.append(item)

        return season_dirs

    def find_episodes(self, directory: Path) -> list[tuple[Path, dict]]:
    ...
    def parse_episode_info(self, filename: str) -> dict | None:
    ...
    def is_tv_directory(self, directory: Path) -> bool:
        ...
        # Check for episode pattern in video files
        for file_path in directory.iterdir():
            if file_path.is_file() and self.is_video_file(file_path):
                if self.parse_episode_info(file_path.name):
                    return True
